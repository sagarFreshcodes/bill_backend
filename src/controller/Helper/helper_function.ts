...
export async function GetUserRecord<T extends ObjectLiteral>(
  ...
  try {
    ...
    const [list, count] = await repository
      ...
      .take(limit)
      .orderBy(
        `${fieldName}`.includes(`role`) ? "role" : fieldName,
        order,
        "NULLS LAST"
      )
      .getManyAndCount();
    const updateList = list.map((i) => {
      return { ...i, role: JSON.parse(i.role) || i.role };
    });

    SuccessResponce(
      res,
      { data: updateList, totalRecords: count },
      messageData.USER_GET_SUCCESSFULL
    );
    return null;
  } catch (error) {
      ...
      .then((update: any) => {
        if (update.raw.length != 0) {
          SuccessResponce(
            res,
            update.raw[0],
            messageData.USER_UPDATE_SUCCESSFULL
          );
        } else {
...
export function ObjectWithRequireKeysValue(
  inputObj: any,
  keysArray: { [key: string]: any }[]
): any {
  const resultObj: any = {};
...
export function FilterObjectsForValidDatabaseField(
  inputArray: any[],
  ...
  return inputArray
    .filter((obj) => {
      ...
      return (
        obj[idKey] !== null && obj[idKey] !== undefined && obj[idKey] !== ""
      );
    })
    .map((obj) => {
      ...
      keysToKeep.forEach((key) => {
        ...
        switch (key) {
          case "category_id":
            const relateIds: number[] | never[] = extractNumbersFromString(
              `${obj[key]}`
            );
      ...
      fs.readFile(csvFile.file.tempFilePath, "utf8", (err, data) => {
        ...
        parse.parse(data, {
          ...
          complete: function (results) {
            ...
            const data = FilterObjectsForValidDatabaseField(
              results.data,
              options
            );
            resolve(data);
...
export function TransformObjectsWithSelectedKey(
  inputArray: any,
  keysToKeep: any
) {
...
export const KeyWiseFilterData = (filterData: any) => {
  ...
  filterData.map(
    (i: { fieldname: any; value: any }) =>
      (filterValues[`${i.fieldname}_values`] = i.value)
  );
  return filterValues;
};

export function ExtractFilterArrayWithKey(
