...
export const Get_attribute = async (req: Request, res: Response) => {
  ...
  try {
    ...
    GetRecord(attributeRepo, res, Attribute, objectForAdd, messageData.ATTRIBUTE_ADD_SUCCESSFULL, {relativeField:relativeField, isFilter: Object.keys(ExtractFilterArray).length != 0, filterValue: filterValue, filterData: ExtractFilterArray, modelName: "Attribute" },)
  } catch (error) {
