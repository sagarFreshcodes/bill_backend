...
export const Get_attribute_value = async (req: Request, res: Response) => {
  const keysArray = [
    // { limit: 10 },
    // { pageNo: 1 },
    // { orderBy: {} },
  ...
  try {
    ...
    const addConditionsForSearch = ` OR cast(categories.id as varchar) ILIKE :searchVal`;
    GetAttributesValueRecord(
      attributeRepo,
      res,
      Attribute,
